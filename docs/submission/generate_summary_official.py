...
S2_LEFT = [
    ("臓器移植マッチングは計算能力だけでは解決できない。**病院が医療データを共有せず、最適化すべきデータが揃わない**ことが根本原因。", 0),
    ("**データ共有を阻む3層の壁:**", 0),
...
S2_RIGHT = [
    ...
    ("hyde暗号文は鍵なしでは乱数と計算量的に区別不能 → 個人情報保護法の「特定個人を識別できる情報」に該当しない", 0),
...
S7_IMPACT = [
    ("**社会:** 待機中死亡の減少、恣意性排除と公平性の数学的保証、データ主権を個人へ。 **経済:** 2030 +44.5〜67億円/年、2040 +2,000億円/年、2050 +7,500億円/年(全臓器マッチ効率+10〜15%、移植1件≈5,000万円)", 0),
]
S7_EXPAND = [
...
def build_slide3_diagram(slide):
    ...
    _box(slide, rx, 2.16, rw, 1.46,
         ...
         "暗号文は鍵なしでは乱数と区別不能＝「個人情報」に該当しない", ORANGE, bsz=10)
