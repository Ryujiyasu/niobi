...
S5_LEFT = [
    ...
    ("**多臓器(負の結果):** 救命数では分離可能=smart greedy=QUBO=厳密最適、量子優位なし。**交換cycle-cover(本命):** QUBOが貪欲に**8/8勝利**・厳密最適に一致・差は規模で拡大(+1.6→+3.7移植)", 0),
...
S6 = [
    ...
    ("FTQC(2030-2040)で大規模化: 問題構造はSAで実証済、実機での求解時間・解品質の比較は今後の課題／N≥10,000へは古典手法との併用で拡張", 0),
