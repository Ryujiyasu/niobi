...
各国のデータ主権は完全に保たれたまま、世界で1つのプールが成立する。
...
qmed（hyde/argo/plat）は同じ構造を **不透明なまま** 実現する。
